  ...
  class ub06_roundtrip_sequence extends uvm_sequence #(ub06_item);
    ...
    task body();
      ...
      for (int unsigned i = 0; i < 3; i++) begin
        ...
        if (!$cast(rsp, base_rsp))
          `uvm_fatal("UB06_TYPE", "response has the wrong transaction type")
        if (rsp.tag != req.tag || rsp.result != (req.operand + 1'b1))
          `uvm_fatal("UB06_RESPONSE", "response does not match its request")
        verified_count++;
      end
    endtask
  endclass
