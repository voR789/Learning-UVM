package dv01_pkg;
    ...
    function new(string name = "dv01_observation");
        super.new(name);
    endfunction
    endclass
        ...
        class dv01_publisher extends uvm_component;
            ...
            task run_phase(uvm_phase phase);
                ...
                observation = dv01_observation::type_id::create("observation1");

                // Intentionally flawed environment: diagnose before changing this method.
                // Issue, not changing our handle object, such that our subscribers recieve the same object alias
                observation.result = 3;
                `uvm_info("DV01_PUBLISH",
                    $sformatf("index=0 result=%0d handle=%0d",
                        observation.result, observation.get_inst_id()), UVM_LOW)
                observed_ap.write(observation);


                observation = dv01_observation::type_id::create("observation2");
                observation.result = 7;
                `uvm_info("DV01_PUBLISH",
                    $sformatf("index=1 result=%0d handle=%0d",
                        observation.result, observation.get_inst_id()), UVM_LOW)
                observed_ap.write(observation);
            endtask
        endclass
        ...
        class dv01_scoreboard extends uvm_component;
            ...
            task run_phase(uvm_phase phase);
                ...
                for (int unsigned index = 0; index < 2; index++) begin
                    ...
                    `uvm_info("DV01_CHECK",
                        $sformatf("index=%0d expected=%0d observed=%0d handle=%0d",
                            index, expected[index], observation.result,
                            observation.get_inst_id()), UVM_LOW)
                    checked++;
                    if (observation.result != expected[index]) begin
                        mismatches++;
                        `uvm_error("DV01_MISMATCH",
                            $sformatf("index=%0d expected=%0d observed=%0d",
                                index, expected[index], observation.result))
                    end
                end
            endtask
        ...
        class dv01_env extends uvm_env;
            ...
            dv01_publisher publisher;
            dv01_scoreboard scoreboard;
            function new(string name, uvm_component parent);
                super.new(name, parent);
            endfunction
        ...
        class dv01_test extends uvm_test;
            ...
            function void build_phase(uvm_phase phase);
                super.build_phase(phase);
                env = dv01_env::type_id::create("env", this);
            endfunction
            task run_phase(uvm_phase phase);
                phase.raise_objection(this);
                wait (env.scoreboard.checked == 2);
                #1ns;
                if (env.scoreboard.mismatches != 0)
                    `uvm_error("DV01_RESULT",
                    $sformatf("checked=%0d mismatches=%0d",
