...
| TC-WRAP-ADD-01   | REQ-OP0                                                 | Test basic wrapping add functionality with and without overflow.                                                                                  | Start at reset state. Drive one wrapping-add command constrained to a non-overflowing sum, then one constrained to an overflowing sum.                                                                                                                                                                         | Transferred response tag, status, and data for each command.          | Both commands should be accepted, follow latency and handshake protocols, and produce two matching responses with status `0`; data should equal the wrapped sum of `a` and `b`.                                                                                                                                    | A response fails to propagate or has incorrect response fields.                                                                              | Operation 0 with overflow and non-overflow values; `op0 x status0` coverage.                                                                   |
| TC-XOR-01        | REQ-OP1                                                 | Test bitwise XOR command.                                                                                                                         | Start at reset state. Drive one command with randomized `a` and `b`.                                                                                                                                                                                                                                           | Transferred response tag, status, and data.                            | The command should be accepted, follow latency and handshake protocols, and produce one matching response with status `0` and data equal to `a ^ b`.                                                                                                                                            | The response fails to propagate or has incorrect response fields.                                                                             | Operation 1; `op1 x status0` coverage.                                                                                                        |
| TC-SAT-ADD-01    | REQ-OP2                                                 | Test basic saturating add functionality with and without saturation.                                                                              | Start at reset state. Drive one non-saturating-add command constrained to a non-overflowing sum, then one saturating-add command constrained to an overflowing sum.                                                                                                                                                | Transferred response tag, status, and data for each command.          | Both commands should be accepted, follow latency and handshake protocols. The non-overflow response should have status `0` and the sum; the overflow response should have status `1` and data `8'hFF`.                                                                                              | A response fails to propagate or has incorrect response fields.                                                                              | Operation 2 with non-saturating and saturating values; `op2 x status0/status1` coverage.                                                      |
...
- uvm_sequence_item: cmd_req
  ...
  - cmd_a[7:0]
  - cmd_b[7:0]
  - drive policy enum: `HOLD` (default) or `PULSE`
...
- `PULSE` is a deliberate one-cycle command probe; it creates no predictor expectation unless a command handshake occurs.
...
- uvm_driver: tcs_rsp_driver (tcs_rsp_sequencer)
  ...
  - Drives `rsp_ready` requests; no UVM sequence response is required by default.
